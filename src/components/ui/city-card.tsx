...
export function CityCard({ city }: { city: CityMeta }) {
  return (
    <Link
      href={`/${city.id}`}
      className="group flex flex-col rounded-[var(--radius-card)] border border-black/8 bg-white p-5 shadow-[var(--shadow-card)] transition-all duration-200 hover:-translate-y-px hover:border-accent/25 hover:shadow-[var(--shadow-hover)]"
    >
      <div className="flex items-start justify-between gap-2">
        <div>
          <h2 className="font-sans text-lg font-bold text-misa-900 group-hover:text-accent">
            {city.name}
          </h2>
