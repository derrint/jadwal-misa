...
function TimeRow({ children }: { children: React.ReactNode }) {
  return (
    <div className="flex items-center gap-2 text-base text-misa-800">
    ...
    >
      ...
      >
        <div className="flex min-w-0 flex-1 items-start gap-3">
          <div className="mt-0.5 flex h-[38px] w-[38px] shrink-0 items-center justify-center rounded-[10px] border border-black/8 bg-accent/10 text-xl">
            ⛪
          </div>
          <div className="min-w-0 flex-1">
            <h2 className="font-serif text-lg font-semibold leading-snug text-misa-900">
              {p.cardName}
            </h2>
            <p className="mt-0.5 text-base font-medium tracking-wide text-accent">
              {p.cardParoki}
            </p>
            <p className="mt-1 text-sm text-misa-500">{p.location}</p>
          </div>
        </div>
        <div className="flex shrink-0 flex-col items-end gap-2">
          <TrustBadge trust={p.trust} />
          <span
            className="text-sm text-misa-400 transition-transform duration-300"
            ...
            {!p.noSchedule ? (
              <div className="mb-4 grid grid-cols-1 gap-3 sm:grid-cols-2">
                <div className="rounded-[10px] border border-black/8 bg-misa-50/80 p-3">
                  <div className="mb-2 flex items-center gap-1 text-sm font-semibold uppercase tracking-wide text-accent">
                    🕯 Misa Sabtu
                  </div>
            ...
            {p.sources?.length ? (
              <div className="mt-3 flex flex-wrap items-center gap-2">
                {p.sources.map((t) => (
                  <span
                    key={t}
                    className="inline-flex items-center gap-1 rounded-md border border-black/8 bg-misa-50 px-2 py-1 text-sm text-misa-500"
                  >
                    {t}
                  </span>
                ))}
