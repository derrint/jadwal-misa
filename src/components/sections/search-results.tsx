    ...
    <section className="mb-10">
      <div className="relative">
        ...
        <input
          ...
          autoComplete="off"
          className="w-full rounded-xl border border-black/8 bg-surface py-3 pl-10 pr-3 text-base text-misa-800 shadow-[var(--shadow-card)] outline-none placeholder:text-misa-400 focus:border-accent focus:ring-2 focus:ring-accent/25"
        />
      </div>
      ...
      {query.trim() ? (
        <ul className="mt-4 space-y-2">
          {filtered.length === 0 ? (
            <li className="rounded-xl border border-black/8 bg-surface p-6 text-center text-base text-misa-500">
              Tidak ada hasil.
            </li>
          ) : (
            filtered.map((e) => (
              <li key={`${e.cityId}-${e.id}`}>
                ...
                >
                  <div className="flex flex-wrap items-start justify-between gap-2">
                    <div>
                      <p className="text-base font-semibold text-misa-900">
                        {e.cardName}
                      </p>
                      <p className="text-base text-accent">{e.cardParoki}</p>
                      <p className="mt-1 text-sm text-misa-500">{e.cityName}</p>
                    </div>
